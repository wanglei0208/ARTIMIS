...
def get_args():
    parser = argparse.ArgumentParser(description='ZOO Black-box Attack via Substitute Model')
    ...
    parser.add_argument('--binary-steps', type=int, default=5)
    parser.add_argument('--nb-parallel', type=int, default=10)
...
def build_zoo_attack(surrogate_model, device, args, input_dim):
...
def main(args):
    ...
    for batch_idx, (data, label, _) in enumerate(tqdm(dataloader)):
        ...
        attack = build_zoo_attack(surrogate_model, device, args, data.shape[1])
