...
from train import get_model, evaluate_model 

def main(args):
    """Main function to run SWA fine-tuning for LSTM models."""
    ...
    model = get_model(args.model_name, input_size=X_train.shape[1], hidden_size=args.hidden_size, num_layers=args.num_layers, output_size=2).to(device)
    ...
    for epoch in range(args.finetune_epochs):
        ...
        for x, y in progress_bar:
            ...
            # The LSTM model does not need flattening, so the view() call is removed.
            ...
            with torch.no_grad():
                for i, param in enumerate(model.parameters()):
                    ...
                    if param.shape == grad_list[i].shape: param.grad = grad_list[i].detach().clone()
            optimizer.step()
    ...
    # We can reuse the evaluate_model from train.py, but it returns more values than needed here.
    # A dedicated, simpler one is also fine. Let's create one inside this script for clarity.
    ...
    acc, prec, rec, f1 = evaluate_final(final_model, test_loader, device)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Fine-tune an LSTM model with optional SWA.")
    # Path and Model Arguments
    parser.add_argument('--model_name', type=str, default='LSTM', help='Name of the model architecture.')
    parser.add_argument('--model_ckpt', type=str, default='../../checkpoints/LSTM/2018/brute_force/LSTM_h120_n3/models/best_f1_model.pth', help='Path to the pre-trained model checkpoint.')
    parser.add_argument('--save_path', type=str, default='../../checkpoints/LSTM/2018/brute_force/LSTM_h120_n3/models/swa_model.pth', help='Path to save the fine-tuned model.')
    ...
    parser.add_argument('--finetune_epochs', type=int, default=25, help='Number of epochs for fine-tuning.')
